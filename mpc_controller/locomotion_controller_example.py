...
# base_file_path = os.path.join(sys.path[0], 'mpc_controller')
# base_file_path = '.\mpc_controller'
# Creating Bots

# bot1_config_file_path = os.path.join(base_file_path, 'bots', 'bot_config', 'bot1.json')
# bot2_config_file_path = os.path.join(base_file_path, 'bots', 'bot_config', 'bot2.json')
...
def _run_example(max_time=_MAX_TIME_SECONDS):
  ...
  # p.loadURDF(os.path.join(base_file_path, 'assets', 'urdfs', 'cylinder.urdf'), 
  #             pos_obs, p.getQuaternionFromEuler([0,0,0]))

  # p.loadURDF("assets/urdfs/f10_racecar/racecar.urdf", 
  #             pos_obs, 
  #             p.getQuaternionFromEuler([0,0,0]))
  
  # p.loadURDF("assets/urdfs/portal.urdf", 
  #             pos_obs, 
  ...
  # robot_sim.START_POS[0] = -10.
  
  robot_uid = p.loadURDF(robot_sim.URDF_NAME, [10., 0., 0.42]) #robot_sim.START_POS)
  ...
  # gamma = 1
  # qp = QP_Controller_Unicycle(gamma, obs_radius= 0.15)
  
  while current_time < max_time:
    pos,orn = p.getBasePositionAndOrientation(robot_uid)
    vel,ang_v = p.getBaseVelocity(robot_uid)
    ...
    # acc, ang_acc = CTRL0.compute_control(current_position=pos_x_y,
    ...
    # qp.setup_QP(bot, [pos_obs[0], pos_obs[1]], [0,0]) #  
    ...
    # lin_speed[0] += ACC[0]*(1/SIM_FREQ)
    # ang_speed += ACC[1]*(1/SIM_FREQ)
    # print("lin_speed=", lin_speed, " ang_speed=", ang_speed)
    
    lin_speed, ang_speed = (0.1, 0., 0.), 0.
